...
export function ButtonSOS() {
  ...
  const getLocation = async () => {
    try {
      ...
      console.log(
        typeof location.coords.longitude,
        typeof location.coords.latitude
      );
  ...
  const manejarPresion = async () => {
    ...
    // llamamos a la función para crear el incidente
    try {
      await createIncident(lng, lat);
      Alert.alert(
        "Emergencia enviada",
        "Tu emergencia ha sido enviada correctamente."
      );
    } catch (error) {
      Alert.alert("Error no se pudo enviar la emergencia", error.message);
      setPressed(false);
      setShowMap(false);
      console.log("Error al crear el incidente:", error);
    } finally {
      setLoading(false);
    }
  };
